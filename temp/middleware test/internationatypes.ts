...
export type ParamsObject<Value extends LocaleValue> = Record<
  Params<Value>[number],
  LocaleValue
>
...
export type Scopes<Locale extends BaseLocale> = ExtractScopes<
  Extract<keyof Locale, string>
>[number]
...
type SomeKey<T extends Record<string, any>> = UnionToTuple<
  keyof T
>[0] extends string
...
export type GetLocaleType<Locales extends ImportedLocales | ExplicitLocales> =
...
type RemovePlural<Key extends string> =
  Key extends `${infer Head}#${PluralSuffix}` ? Head : Key
...
> =
  IsPlural<Key, Scope, Locale> extends true
    ? AddCount<
        GetParams<Value>['length'] extends 0 ? [] : [T],
        Key,
        Scope,
        Locale
      >
